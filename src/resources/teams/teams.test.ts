...
describe("getTeams", () => {
  ...
  it("should include the full roster if specified", async () => {
    const activeRosterResults = await getTeams({
      id: teamId,
      expand: ["team.roster"],
    });
  ...
  it.each<TeamExpand>(["roster.person", "person.names", "person.social"])(
    "should include the roster if %s expand is specified",
    async (expand) => {
      const results = await getTeams({ id: teamId, expand: [expand] });
  ...
  it.each<[keyof NHLPlayer, TeamExpand]>([
    ...
    "should include player %s in the roster if specified",
    async (property, expand) => {
      const results = await getTeams({
        id: teamId,
  ...
  it.each<TeamExpand>(["person.names", "person.social"])(
    "should include expanded player info if %s expand is specified",
    async (expand) => {
      const results = await getTeams({
        id: teamId,
...
describe("getTeamRoster", () => {
  ...
  it("should include the full roster if specified", async () => {
    ...
    const fullRosterResults = await getTeamRoster({
      id: teamId,
      rosterType: "fullRoster",
    });

    expect(fullRosterResults.length).toBeGreaterThan(
      activeRosterResults.length
    );
  });

  it("should include expanded player info in the roster if specified", async () => {
    const results = await getTeamRoster({
      id: teamId,
      expand: ["roster.person"],
    });
  ...
  it.each<TeamRosterExpand>(["person.names", "person.social"])(
    "should include expanded player info if %s expand is specified",
    async (expand) => {
